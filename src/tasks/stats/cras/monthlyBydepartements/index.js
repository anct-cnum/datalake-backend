...
execute(__filename, async ({ logger, dbDatalake }) => {
  ...
  try {
    ...
    await statsAlltasks.getStatsDoms(dbDatalake, query);

    //Cas ST MARTIN (97150)
    await statsAlltasks.getStatsStMartin(dbDatalake, query);

    //Cas Corse 2A ('200XX', '201XX')
    await statsAlltasks.getStatsCorse2A(dbDatalake, query);

    //Cas Corse 2B ('202XX', '206XX')
    await statsAlltasks.getStatsCorse2B(dbDatalake, query);

    //Les autres départements
    await statsAlltasks.getStatsAllOthers(dbDatalake, query);
    ...
    const depsListFormatted = await statsAlltasks.getListDepsFormatted(dbDatalake, departements, nouvelleColonne);
    ...
    depsListFormatted.forEach(departement => {
      promises.push(new Promise(async resolve => {
        ...
        await dbDatalake.collection('stats_departements_cras').updateOne(queryUpd, update, options);
        resolve();
      }));
    });
    ...
    await dbDatalake.collection('temporary_corse2a_stats_departements_cras').drop();
    await dbDatalake.collection('temporary_corse2b_stats_departements_cras').drop();
    await dbDatalake.collection('temporary_doms_stats_departements_cras').drop();
    await dbDatalake.collection('temporary_others_stats_departements_cras').drop();
    await dbDatalake.collection('temporary_stmartin_departements_cras').drop();
