...
const idCnMatchCraSansStructureId = dbDatalake => async arrayIdConseiller => {
  const resultDatalake = await dbDatalake.collection('cras').distinct('conseillerId', { 'structureId': { '$exists': false } });
...
// Partie 2 => find des cras concernés à mettre à jour
const getCrasToConseiller = db => async id => await db.collection('cras').find({
  'conseiller.$id': id,
  'createdAt': { $lte: new Date('2022-12-01') }
}).toArray();
const getIdCrasSansStructureId = dbDatalake => async id => await dbDatalake.collection('cras').distinct('_id',
...
execute(__filename, async ({ logger, db, dbDatalake }) => {
  ...
  try {
    ...
    conseillerCraToUpdateLimit.forEach(obj => {
      promises.push(new Promise(async resolve => {
        ...
        const getCraSansStructureIdToConseiller = await getIdCrasSansStructureId(dbDatalake)(obj.idConseillerDatalake);
        const filtreCraSansIdSA = crasCnfs.filter(e => getCraSansStructureIdToConseiller.includes(encrypt(e._id.toString())));
  ...
  logger.info(`${modifiedCount} / ${conseillerCraToUpdate.length} conseiller(s) concerné(s), ont eu des CRAs mis à jour`);
});
